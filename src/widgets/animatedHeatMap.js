import processStructure from '@devoinc/applications-data-library/structures/animatedMap';
import widgetFactory from '@devoinc/applications-builder/widgetFactory';
import dependencies from '../data/dependencies';

const AnimatedHeatMapWidget =
  dependencies.require('widgets').AnimatedHeatMapWidget;

/**
 * This chart displays data animated over time, represented as heat points
 * and geolocated on a world map using coordinates (latitude and longitude).
 * The higher the values the brighter the color and bigger the area of the
 * heat points.
 * @category Widgets
 * @module AnimatedHeatmap
 ...
 * @tutorial widgets-animated-heatmap
 */
function mixin(self) {
  return {
    /**
     ...
     * @instance
     */
    setDissipating: (disipating) => (self.settings.disipating = disipating),
     ...
     * @param {number} number - Time in milliseconds.
     * @default 500
     * @instance
     */
    setPeriodDelay: (period) => (self.settings.periodDelay = period),
    ...
    render: (orig) => {
      ...
      let data = processStructure(orig, cfg.keys, cfg.value);
      if (data) {
        self.widget = new AnimatedHeatMapWidget(cfg);
